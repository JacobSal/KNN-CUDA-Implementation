...
class PanZoomWindow(object):
    ...
    def display_lines(self):
        ...
        count = 0
        for lines in self.points:
            if lines == []:
                pass
            else:
                pointstate = np.array(self.points_display[count]).reshape((-1,1,2))
                if lines[1] == "l":
                    if lines[0]:
                        ...
                        cv2.polylines(self.img[int(pzs.ul[0]):int(pzs.ul[0]+pzs.shape[0]), int(pzs.ul[1]):int(pzs.ul[1]+pzs.shape[1])] , [pointstate],False ,[100,100,0], 1)
                elif lines[1] == "f":
                    if lines[0]:
                        cv2.fillPoly(self.img[int(pzs.ul[0]):int(pzs.ul[0]+pzs.shape[0]), int(pzs.ul[1]):int(pzs.ul[1]+pzs.shape[1])], [pointstate],[255,0,0])
                    else:
                        cv2.fillPoly(self.img[int(pzs.ul[0]):int(pzs.ul[0]+pzs.shape[0]), int(pzs.ul[1]):int(pzs.ul[1]+pzs.shape[1])], [pointstate],[100,100,0])
            count += 1
        cv2.imshow(self.WINDOW_NAME,self.img[int(pzs.ul[0]):int(pzs.ul[0]+pzs.shape[0]), int(pzs.ul[1]):int(pzs.ul[1]+pzs.shape[1])])
    'end def'
    
    def gen_point_vector(self):
        point_data = np.zeros((self.DEFAULT.shape[0]*self.DEFAULT.shape[1],2))
        count = 0
        for i in range(0,self.DEFAULT.shape[0]):
            for j in range(0,self.DEFAULT.shape[1]):
    ...
    def get_nonzeros(self,val_vector,mask,tru_type = True):
        mask = mask.ravel()
        mask = mask.reshape(mask.shape[0],1)
        
        mask_new = np.zeros((mask.shape[0],mask.shape[1]))
        points_new = np.zeros((mask.shape[0],mask.shape[1]))
        
        points = self.gen_point_vector()
        vals_new = np.zeros((val_vector.shape[0],val_vector.shape[1]+1))
        
        count = 0        
        for i,x in enumerate(mask):
            if x.any() != 0:
                vals_new[count,:-2] = val_vector[i,:]
    ...
    def gen_training_list(self,*kwargs):
        bitimage = np.zeros((nW,nH)).astype(np.float32)
        for i in range(0,len(self.points)):
            if self.points[i] == []:
                pass
            else:
                line = np.array(self.points[i][2:]).reshape((-1,1,2))
                if self.points[i][1] == 'l':
                    if self.points[i][0]:
                        cv2.polylines(bitimage,[line],False,[1,1,1],1)
                    else:
                        cv2.polylines(bitimage,[line],False,[2,2,2],1)
                elif self.points[i][1] == 'f':
                    if self.points[i][0]:
                        cv2.fillPoly(bitimage,[line],[1,1,1])
                    else:
                        cv2.fillPoly(bitimage,[line],[2,2,2])
        'end for'
        con_data = self.DEFAULT.ravel()
        con_data = con_data.reshape(con_data[0],1)
        ...
        Tmask,Tval,Tpoint = self.get_nonzeros(con_data,bitimage==1,True)
        Fmask,Fval,Fpoint = self.get_nonzeros(con_data,bitimage==2,False)
        
        T_data = np.concatenate((Tval,Fval),axis = 0)
        T_point = np.concatenate((Tpoint,Fpoint),axis = 0)
        T_mask = np.concatenate((Tmask,Fmask),axis = 0)
        
        return T_data,T_mask,T_point
    
    def inter_Polate(self,ML_func,*kwargs):
        
        X_,y_,points = self.gen_training_list(direction_features,ffhi)
        C = 1.0  # SVM regularization parameter
        models = svm.SVC(kernel='poly',degree = 2, gamma = 'auto', C=C)
        start = time.time()
        fitted = models.fit(X_,y_)
        end = time.time()
        print(end-start)
        
        predictions = fitted.predict(X2)
    ...
    def menuChange(self):
        if cv2.waitKey(0) == ord('m'):
            ...
            if cv2.waitKey(0) == ord('p'):
                ffhi = Filters.FFfilt(self.DEFAULT,0,250,False)
                direction_features = Filters.diffmat(ffhi,np.arange(0,2*np.pi,2*np.pi/16),dim=(5,4))
